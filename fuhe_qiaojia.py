...
def data_dealer(datas):
    ...
    for val in datas:
        #先处理桥架本体
        if val.find('直通') != -1 or val.find('弯通') != -1 or val.find('三通') != -1 or val.find('四通') != -1 or val.find('凹') != -1 or val.find('异径接头') != -1 or val.find('凸') != -1 and val.find('抱箍') == -1 and val.find('封头') == -1 and val.find('终端头') == -1 and val.find('管接头') == -1 and val.find('扎带') == -1 and val.find('隔板') == -1 and val.find('直接片') == -1 and val.find('连接板') == -1 and val.find('弯接片') == -1 and val.find('调角片') == -1 and val.find('调宽片') == -1 and val.find('伸缩片') == -1 and val.find('调高片') == -1 and val.find('连接线') == -1 and val.find('接地线') == -1 and val.find('跨接线') == -1 and val.find('接连线') == -1 and val.find('固定压板') == -1 and val.find('胶垫') == -1:
            val = re.sub(r'X', '×', val)
            val = re.sub(r'x', '×', val)
            val = re.sub(r'\*', '×', val)
            ...
            if val.find('抱箍') != -1:
                classes = 'baoku'
                features = fujian_fuhe.baoku(vals)
            elif val.find('封头') != -1 or val.find('终端头') != -1:
                classes = 'fengtou'
                features = fujian_fuhe.fengtou(vals)
            elif val.find('管接头') != -1:
                classes = 'guanjietou'
                features = fujian_fuhe.guanjietou(vals)
            elif val.find('扎带') != -1:
                classes = 'zadai'
                features = fujian_fuhe.zadai(vals)
            elif val.find('隔板') != -1:
                classes = 'geban'
                features = fujian_fuhe.geban(vals)
            elif val.find('直接片') != -1 or val.find('伸缩片') != -1 or val.find('连接片') != -1 or val.find('连接板') != -1:
                classes = 'zhijiepian'
                features = fujian_fuhe.zhijiepian(vals)
            elif val.find('弯接片') != -1:
                classes = 'wanjiepian'
                features = fujian_fuhe.wanjiepian(vals)
            elif val.find('调角片') != -1 or val.find('调高片') != -1 or val.find('调宽片') != -1:
                classes = 'tiaojiaopian'
                features = fujian_fuhe.tiaojiaopian(vals)
            elif val.find('连接线') != -1 or val.find('接地线') != -1 or val.find('跨接线') != -1 or val.find('接连线') != -1:
                classes = 'lianjiexian'
                features = fujian_fuhe.lianjiexian(vals)
            elif val.find('固定压板') != -1:
                classes = 'gudingyaban'
                features = fujian_fuhe.gudingyaban(vals)
            elif val.find('胶垫') != -1:
                classes = 'xiangjiaodian'
                features = fujian_fuhe.xiangjiaodian(vals)
            elif val.find('伸缩片') != -1:
                classes = 'shensuopian'
                features = fujian_fuhe.shensuopian(vals)
            else:
                classes = 'error'
                features = ['', '', '', '', '', 'error','']
            print(vals)
